// Genera la suscripción estable del Benjamín B a partir de los datos oficiales.
const fs = require('node:fs');
const data = JSON.parse(fs.readFileSync('data/benjamin-b-rfaf.json', 'utf8'));
...
for (const match of data.matches) {
  ...
  lines.push('BEGIN:VEVENT', 'UID:zabal-benjamin-b-' + match.round + '@atleticozabal.com',
    'DTSTAMP:' + stamp);
  if (match.time) {
  ...
  const notes = ['Tercera Andaluza Cádiz Benjamín · Grupo 4', 'Jornada ' + match.round,
  ...
  lines.push('URL:https://www.atleticozabal.com/calendario-benjamin.html', 'END:VEVENT');
}
lines.push('END:VCALENDAR');
fs.writeFileSync('calendario-benjamin.ics', lines.map(fold).join('\r\n') + '\r\n');
