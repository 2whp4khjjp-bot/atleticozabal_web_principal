...
const source = base + '/pnfg/NPcd/NFG_VisCalendario_Vis?cod_primaria=1000120&codtemporada=22&codcompeticion=48893775&codgrupo=48894181&CodJornada=1&CDetalle=1';
const group = base + '/pnfg/NPcd/NFG_VisGrupos_Vis?cod_primaria=1000123&codcompeticion=48893775&codgrupo=48894181';
...
(async () => {
  ...
  try {
    ...
    if (matches.length < 2 ||
      ...
      throw new Error('El calendario del Alevín A no coincide con el grupo esperado');
    }
    await attachRfafActas(page, matches, { base, competition: '48893775', group: '48894181' });
    const classificationUrl = base + '/pnfg/NPcd/NFG_VisClasificacion?cod_primaria=1000120&codgrupo=48894181&codcompeticion=48893775';
    ...
    if (!classificationResponse.ok ||
      ...
      throw new Error('No se puede verificar la clasificación del Alevín A');
    }
    ...
    mergeStoredScores('data/alevin-a-rfaf.json', matches);
    const output = { source, classificationSource: classificationUrl,
      updatedAt: new Date().toISOString(), standing, matches };
    fs.mkdirSync('data', { recursive: true });
    fs.writeFileSync('data/alevin-a-rfaf.json', JSON.stringify(output, null, 2) + '\n');
