...
const data = JSON.parse(fs.readFileSync('data/cadete-rfaf.json', 'utf8'));
...
const lines = ['BEGIN:VCALENDAR', 'VERSION:2.0',
  'PRODID:-//Atletico Zabal Linense//Cadete 2026-27//ES',
  'CALSCALE:GREGORIAN', 'METHOD:PUBLISH',
  'X-WR-CALNAME:Atlético Zabal · Cadete',
...
for (const match of data.matches) {
  ...
  lines.push('BEGIN:VEVENT', 'UID:zabal-cadete-' + match.round + '@atleticozabal.com',
    'DTSTAMP:' + stamp);
  if (match.time) {
  ...
  const notes = ['Cadete · Segunda Andaluza Cádiz · Grupo único',
    'Jornada ' + match.round,
  ...
  lines.push('URL:https://www.atleticozabal.com/calendario-cadete.html', 'END:VEVENT');
}
lines.push('END:VCALENDAR');
fs.writeFileSync('calendario-cadete.ics', lines.map(fold).join('\r\n') + '\r\n');
