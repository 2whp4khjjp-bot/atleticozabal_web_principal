...
const group = base + '/pnfg/NPcd/NFG_VisGrupos_Vis?cod_primaria=1000123&codcompeticion=48909139&codgrupo=48909177';
...
(async () => {
  ...
  try {
    ...
    if (matches.length < 2 ||
      ...
      throw new Error('El calendario del Infantil A no coincide con el grupo esperado');
    }
    ...
    mergeStoredScores('data/infantil-a-rfaf.json', matches);
    const output = { source, classificationSource: classificationUrl,
      updatedAt: new Date().toISOString(), standing, matches };
    fs.mkdirSync('data', { recursive: true });
    fs.writeFileSync('data/infantil-a-rfaf.json', JSON.stringify(output, null, 2) + '\n');
