...
const data = JSON.parse(fs.readFileSync('data/alevin-b-rfaf.json', 'utf8'));
...
const lines = ['BEGIN:VCALENDAR', 'VERSION:2.0',
  'PRODID:-//Atletico Zabal Linense//Alevin B 2026-27//ES',
  'CALSCALE:GREGORIAN', 'METHOD:PUBLISH',
  'X-WR-CALNAME:Atlético Zabal · Alevín B',
...
for (const match of data.matches) {
  ...
  lines.push('BEGIN:VEVENT', 'UID:zabal-alevin-b-' + match.round + '@atleticozabal.com',
    'DTSTAMP:' + stamp);
  if (match.time) {
  ...
  const notes = ['Alevín B · Segunda Andaluza Cádiz · Grupo único',
    'Jornada ' + match.round,
  ...
  lines.push('URL:https://www.atleticozabal.com/calendario-alevin-b.html', 'END:VEVENT');
}
lines.push('END:VCALENDAR');
fs.writeFileSync('calendario-alevin-b.ics', lines.map(fold).join('\r\n') + '\r\n');
