...
const data = JSON.parse(fs.readFileSync('data/infantil-a-rfaf.json', 'utf8'));
...
const lines = ['BEGIN:VCALENDAR', 'VERSION:2.0',
  'PRODID:-//Atletico Zabal Linense//Infantil A 2026-27//ES',
  'CALSCALE:GREGORIAN', 'METHOD:PUBLISH',
  'X-WR-CALNAME:Atlético Zabal · Infantil A',
...
for (const match of data.matches) {
  ...
  lines.push('BEGIN:VEVENT', 'UID:zabal-infantil-a-' + match.round + '@atleticozabal.com',
    'DTSTAMP:' + stamp);
  if (match.time) {
  ...
  const notes = ['Infantil A · Segunda Andaluza Cádiz · Grupo único',
    'Jornada ' + match.round,
  ...
  lines.push('URL:https://www.atleticozabal.com/calendario-infantil-a.html', 'END:VEVENT');
}
lines.push('END:VCALENDAR');
const output = lines.map(fold).join('\r\n') + '\r\n';
fs.writeFileSync('calendario-infantil-a.ics', output);
// Mantiene compatible el enlace incorrecto compartido inicialmente.
fs.writeFileSync('calendario-alevin-b.ics', output);
