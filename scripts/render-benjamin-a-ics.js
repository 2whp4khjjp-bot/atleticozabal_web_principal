...
const data = JSON.parse(fs.readFileSync('data/benjamin-a-rfaf.json', 'utf8'));
if (!Array.isArray(data.matches) || data.matches.length !== 30) {
  throw new Error('No se genera ICS sin las 30 jornadas verificadas');
}
...
const lines = ['BEGIN:VCALENDAR', 'VERSION:2.0',
  'PRODID:-//Atletico Zabal Linense//Benjamin A 2026-27//ES',
  'CALSCALE:GREGORIAN', 'METHOD:PUBLISH',
  'X-WR-CALNAME:Atlético Zabal · Benjamín A',
...
for (const match of data.matches) {
  ...
  lines.push('BEGIN:VEVENT', 'UID:zabal-benjamin-a-' + match.round + '@atleticozabal.com',
    'DTSTAMP:' + stamp);
  if (match.time) {
  ...
  const notes = ['Segunda Andaluza Cádiz Benjamín · Grupo único',
    'Jornada ' + match.round,
  ...
  lines.push('URL:https://www.atleticozabal.com/calendario-benjamin-a.html', 'END:VEVENT');
}
lines.push('END:VCALENDAR');
fs.writeFileSync('calendario-benjamin-a.ics', lines.map(fold).join('\r\n') + '\r\n');
