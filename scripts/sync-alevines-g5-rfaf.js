// Extrae los dos alevines del Grupo 5 desde la web pública de la RFAF.
...
const group = base + '/pnfg/NPcd/NFG_VisGrupos_Vis?cod_primaria=1000123&codcompeticion=49286744&codgrupo=49287025';
const classificationSource = base + '/pnfg/NPcd/NFG_VisClasificacion?cod_primaria=1000120&codgrupo=49287025&codcompeticion=49286744&codjornada=1';
const teams = [
  { key: 'alevin-zabal-c', official: 'ATLETICO ZABAL "B"', label: 'Alevín C' },
...
(async () => {
  ...
  try {
    ...
    if (!page.url().includes('NFG_VisCalendario_Vis')) {
      throw new Error('RFAF no abrió el calendario del Grupo 5');
    }
    ...
    for (const round of pastRounds) {
      const resultsUrl = base + '/pnfg/NPcd/NFG_CmpJornada?cod_primaria=1000120' +
        '&CodCompeticion=49286744&CodGrupo=49287025&CodTemporada=22' +
        ...
        const officialScore = await page.evaluate(({ home, away }) => {
          ...
          const readScore = span => {
            ...
            const digits = [...span.querySelectorAll('[id]')].map(element => {
              const digitClass = [...element.classList]
                .find(name => /^fa-\d$/.test(name));
    ...
    if (!response.ok() || !page.url().includes('NFG_VisClasificacion')) {
      throw new Error('RFAF no abrió la clasificación del Grupo 5');
    }
