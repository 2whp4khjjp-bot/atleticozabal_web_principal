// Extrae el Juvenil B desde la web pública de la RFAF.
...
const group = base + '/pnfg/NPcd/NFG_VisGrupos_Vis?cod_primaria=1000123&codcompeticion=49145109&codgrupo=49151057';
const classificationSource = base + '/pnfg/NPcd/NFG_VisClasificacion?cod_primaria=1000120&codgrupo=49151057&codcompeticion=49145109&codjornada=1';
...
(async () => {
  ...
  try {
    ...
    if (!page.url().includes('NFG_VisCalendario_Vis')) {
      throw new Error('RFAF no abrió el calendario del Juvenil B');
    }
    ...
    if (!response || !response.ok() || !page.url().includes('NFG_VisClasificacion')) {
      throw new Error('RFAF no abrió la clasificación del Juvenil B');
    }
    ...
    const standing = teamIndex >= 1 ? {
      position: number(teamIndex - 1), points: number(teamIndex + 2),
      played: number(teamIndex + 3), goalsFor: number(teamIndex + 11),
    ...
    if (!standing || standing.position < 1 || standing.position > 20 ||
      ...
      throw new Error('No se puede verificar la clasificación del Juvenil B');
    }
    ...
    mergeStoredScores('data/juvenil-b-rfaf.json', matches);
    const output = { source, classificationSource, updatedAt: new Date().toISOString(), standing, matches };
    fs.mkdirSync('data', { recursive: true });
    fs.writeFileSync('data/juvenil-b-rfaf.json', JSON.stringify(output, null, 2) + '\n');
    console.log('Juvenil B: ' + matches.length + ' partidos; ' +
