...
(async () => {
  ...
  try {
    ...
    if (!classificationResponse.ok ||
      ...
      throw new Error('No se puede verificar la clasificación del Alevín B');
    }
    ...
    mergeStoredScores('data/alevin-b-rfaf.json', matches);
    const output = { source, classificationSource: classificationUrl,
      updatedAt: new Date().toISOString(), standing, matches };
    fs.mkdirSync('data', { recursive: true });
    fs.writeFileSync('data/alevin-b-rfaf.json', JSON.stringify(output, null, 2) + '\n');
