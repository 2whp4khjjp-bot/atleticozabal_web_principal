...
const json = JSON.parse(fs.readFileSync('data/senior-rfaf.json', 'utf8'));
if (!Array.isArray(json.matches) || json.matches.length < 2) throw new Error('Faltan jornadas del Sénior');
const ics = fs.readFileSync('calendario-senior.ics', 'utf8');
const total = json.matches.length;
if ((ics.match(/BEGIN:VEVENT/g) || []).length !== total ||
    (ics.match(/END:VEVENT/g) || []).length !== total ||
    (ics.match(/UID:zabal-senior-/g) || []).length !== total) {
  throw new Error('El ICS del Sénior no contiene todos los eventos');
}
for (const line of ics.split('\r\n').filter(Boolean)) {
  if (Buffer.byteLength(line) > 75) throw new Error('Línea ICS sin plegar');
}
console.log('Calendario Sénior válido: ' + total + ' partidos.');
